def GS(phi0,f,n,xi,xf,its, dim='1d', graph=True, ax=None):
    ...
    if dim=='1d':
        ...
        for k in range(its):
            ...
            if graph:
                ...
                ax.set_xlabel('x')
                ax.set_ylabel('phi(x)')
                ax.set_title('Gauss-Seidel approximation after '+str(k)+' iterations')
                plt.pause(.002)
                #time.sleep(0.05)
        ...
        for k in range(its):
            for j in range(1,n-1):
                for i in range(1,n-1):
                    phi[j][i]=0.25*(phi[i-1][j]+phi[i+1][j]+phi[i][j-1]+phi[i][j+1] - b[j][i])
                #print len(phi[1]),'*' #(debug)
            if graph:
                if sct is not None:
                    sct.remove()
                X,Y=np.meshgrid(x,y)
        ...
        assert ((dim=='2d') or (dim=='1d')), "GS PyterSolve only supports 1d or 2d"
